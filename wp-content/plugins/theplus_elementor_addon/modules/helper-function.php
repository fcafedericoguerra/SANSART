 ...
 * @since 1.0.0
 * @version 5.6.2
 * @param string $post_type The post type slug. Default is an empty string.
 * @return array List of taxonomies for the specified post type.
 */
function theplus_get_post_taxonomies( $post_type = '',$blog_post = '' ) {
	$args = array(
		'public'  => true,
		'show_ui' => true,
	);
...
function theplus_get_style_list( $max=4, $none='',$custom='' ) {
	...
	if ( 'smart-loop-builder' === $custom ) {
		$options[ $custom ] = esc_html__( 'Smart Loop Builder - Beta', 'theplus' );
	}
