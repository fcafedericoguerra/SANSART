...
class ThePlus_Clients_ListOut extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'label_block' => true,
				'multiple'    => true,
				'options'     => $this->theplus_get_client_categories(),
				'separator'   => 'before',
			)
		);
		$this->add_control(
	...
	protected function render() {
		...
		$clients_name     = $this->theplus_client_post_name();
		$clients_taxonomy = $this->theplus_client_post_category();
		...
		$display_post_title = ! empty( $settings['display_post_title'] ) ? $settings['display_post_title'] : 'yes';
		$disable_link       = ! empty( $settings['disable_link'] ) ? $settings['disable_link'] : 'no';
		...
		if ( ! empty( $clientContentFrom ) && 'clrepeater' === $clientContentFrom ) {
			if ( ! empty( $clientLinkMaskList ) ) {
				...
				if ( 'yes' === $filterCategory ) {
					...
					foreach ( $clientLinkMaskList as $item ) {
						...
						if ( 'yes' === $filterCategory && ! empty( $clientCategory ) && 'carousel' !== $layout ) {
			...
			$output .= '<div id="pt-plus-clients-post-list" class="clients-list ' . esc_attr( $uid ) . ' ' . esc_attr( $data_class ) . '  '.esc_attr($ajax_filter).' ' . $animated_class . '" ' . $layout_attr . ' ' . $data_attr . ' ' . $animation_attr . ' ' . $carousel_slider . ' dir=' . esc_attr( $carousel_direction ) . ' data-enable-isotope="1" filter-type = "'.esc_attr($ajax_filter_type).'">';
			...
			$post_extra_option = ! empty( $settings['post_extra_option'] ) ? $settings['post_extra_option'] : 'none';
			...
			if ( ( 'load_more' === $post_extra_option || 'lazy_load' === $post_extra_option ) && 'carousel' !== $layout || ( 'yes' === $filterCategory && 'ajax_base' === $ajax_filter_type )  ) {
				$postattr     = array(
					'load'               => 'clients',
					...
					'more_post_option'   => esc_attr( $post_extra_option ),
				...
				if ( ( 'yes' === $filterCategory && 'ajax_base' === $ajax_filter_type ) || ( 'load_more' === $post_extra_option && 'carousel' !== $layout ) ) {
			...
			$output .= '<div class="tp-row post-inner-loop ' . esc_attr( $layout_style ) . '  ' . esc_attr( $d_flex ) . ' flex-wrap tp-align-items-center ' . esc_attr( $uid ) . '" '.$extparam.' '.$wooAttr.'>';
			...
			if ( 'pagination' === $post_extra_option && 'carousel' !== $layout ) {
			...
			} elseif ( 'load_more' === $post_extra_option && 'carousel' !== $layout ) {
				...
				$output .= '</div>';
			} elseif ( 'lazy_load' === $post_extra_option && 'carousel' !== $layout ) {
	...
	protected function get_filter_category() {
		...
		if ( ( 'yes' === $filterCategory ) && ( 'clcontent' === $clientContentFrom || 'clrepeater' === $clientContentFrom ) ) {
			...
			if ( ( 'yes' === $filterCategory ) && ( 'clrepeater' === $clientContentFrom ) ) {
				$loop_category = array();
				$count_loop    = 0;
			...
			if ( 'clcontent' === $clientContentFrom ) {
			...
			} elseif ( 'clrepeater' === $clientContentFrom ) {
				if ( 'style-1' === $filter_style ) {
			...
			if ( ! empty( $clientLinkMaskList ) && 'clrepeater' === $clientContentFrom ) {
	...
	protected function get_query_args() {
		...
		if ( 'pagination' !== $settings['post_extra_option'] ) {
			$query_args['offset'] = $offset;
		} elseif ( 'pagination' === $settings['post_extra_option'] ) {
	...
	/**
	 * Get categories of the client post type
	 *
	 * @since 6.1.0
	 */
	public function theplus_get_client_categories() {
		$clients = $this->theplus_client_post_category();
	 ...
	 * @since 6.1.0
	 */
	public function theplus_client_post_category() {
	...
	/**
	 * Get categories of Name.
	 *
	 * @since 6.1.0
	 */
	public function theplus_client_post_name() {
		$post_type_options = get_option( 'post_type_options' );
		$client_post_type  = ! empty( $post_type_options['client_post_type'] ) ? $post_type_options['client_post_type'] : '';
