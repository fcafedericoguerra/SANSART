...
class ThePlus_Testimonial_ListOut extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'label_block' => true,
				'multiple'    => true,
				'options'     => $this->theplus_get_testimonial_categories(),
				'separator'   => 'before',
			)
		);
		$this->add_control(
		...
		$this->add_responsive_control(
			...
			array(
				...
				'render_type' => 'ui',
				'selectors'   => array(
					'{{WRAPPER}} .testimonial-list .testimonial-list-content' => 'height: {{SIZE}}{{UNIT}};overflow-y: auto; padding-right: 5px;',
				),
				'conditions'  => array(
					'relation' => 'or',
					'terms'    => array(
	...
	protected function render() {
		...
		$post_name     = $this->theplus_testimonial_post_name();
		$taxonomy_name = $this->theplus_testimonial_post_category();
		...
		if ( ! empty( $tlContentFrom ) && $tlContentFrom == 'tlrepeater' ) {
			if ( ! empty( $testiAllList ) ) {
				...
				foreach ( $testiAllList as $item ) {
					...
					if ( ! empty( $style ) ) {
							ob_start();
								include THEPLUS_WSTYLES . 'testimonial/testimonial-' . sanitize_file_name( $style ) . '.php';
	 ...
	 * @since 6.1.0
	 */
	public function theplus_get_testimonial_categories() {

		$testimonial = $this->theplus_testimonial_post_category();
	 ...
	 * @since 6.1.0
	 */
	public function theplus_testimonial_post_category() {
	 ...
	 * @since 6.1.0
	 */
	public function theplus_testimonial_post_name() {
		$post_type_options = get_option( 'post_type_options' );
		$testi_post_type   = ! empty( $post_type_options['testimonial_post_type'] ) ? $post_type_options['testimonial_post_type'] : '';
