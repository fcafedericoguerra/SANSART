...
var tp_wise_filter = function (current_widget, get_wooattr, category, type) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            tp_resizelayout(get_row, response);
            ...
            if (type === 'onload') {
                if (response.filter_id) {
                    current_widget.querySelector('.post-filter-data ul li .active').classList.remove("active");
            ...
            if (isloadmore.length > 0) {
                ...
                var click = isloadmore[0],
                    ...
                    offset_posts = click?.dataset?.offset_posts ? click.dataset.offset_posts : 0,
...
var tp_resizelayout = function (html, option) {
    var delayload = 500;
