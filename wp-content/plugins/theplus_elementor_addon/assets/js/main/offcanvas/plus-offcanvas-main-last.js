...
PlusOffcanvas.prototype = {
   ...
   bindEvents: function () {		
       ...
       "yes" === this.body_click_close && this.closeClick();//enable commnet @5.0.8* 
   ...
   show: function () {
       ...
       jQuery("html.plus-" + this.id + "-open .plus-offcanvas-container").on("click", function (a) { 
        ...
        /**TabTour AutoPlay*/
        if( a.target && a.target.classList.contains('plus-tab-header') ){
            return;
        }

           var getPopupCon = document.querySelector('.plus-visible')
           if( getPopupCon && getPopupCon.classList.contains('tp-outer-yes') )
