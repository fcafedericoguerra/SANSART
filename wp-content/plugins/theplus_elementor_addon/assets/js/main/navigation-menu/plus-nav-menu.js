	...
	var WidgetHeaderNavigation = function($scope, $) {
		...
		if($plus_navigation.length > 0){
			...
			if (nav_sticky!=undefined && nav_sticky=='yes' && nav_sticky_id!='') {
				...
				if(inner_width < 767 && $plus_navigation.closest(".elementor-element.elementor-section:not(.elementor-inner-section):not(.elementor-hidden-mobile),.elementor-element.e-container,.elementor-element.e-con").length){
					$plus_navigation.append('<div class="plus-nav-sticky ' + nav_sticky_id + "-" + sec_id + '"></div>');
				}
