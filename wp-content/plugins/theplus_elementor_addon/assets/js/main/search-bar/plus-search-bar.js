...
(function($) {
    ...
    var WidgetSearchBarHandler = function($scope, $) {
        ...
        if(ajaxsearch.ajax == 'yes'){
            ...
            $(container[0]).on("change keyup", function(e){
                let EvnetType = e.type,
                    ...
                    getCat = container[0].querySelectorAll("input[name^=taxonomy_]");
                ...
                if(getCat.length > 0){
                    getCat.forEach(function(self){
                        if( (EvnetType == 'keyup' && serText.length >= $Searhlimit) 
                        || (EvnetType == 'change' && (self && SpecialCTP == 0) || (self && serText.length >= $Searhlimit) || (self=='' && serText.length >= $Searhlimit))) {
                            showhide_search_data(e);
                        }else{
                    ...
                    if( ( EvnetType == 'keyup' && serText.length >= $Searhlimit ) 
                    || ( EvnetType == 'change' && ( (post && SpecialCTP == 0) || (post && serText.length >= $Searhlimit) || (post=='' && serText.length >= $Searhlimit) ) ) ) {
                        showhide_search_data(e)
                    }else{
                        container[0].querySelectorAll('.tp-close-btn');
                    }
        ...
        function showhide_search_data(e){
