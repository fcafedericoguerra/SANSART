		...
		if($TabHover == 'no'){
			$($currentTabId + ' ul.plus-tabs-nav li .plus-tab-header').on('click',function(){
				var currentTabIndex = this.dataset.tab,
					...
					// For auto swiper scroll in header tab for hosrizontal 
					if( container[0].classList.contains('swiper-container') && ( event.isTrusted === false ) ) {
						
						if(event.layerX < 0){
	
							var scWidth = window.screen.width;
	
							var tabsLayerX = event.layerX;
								tabsLayerX = Math.abs(tabsLayerX);
							
							if( tabsLayerX > scWidth ) {
								var finalVal = tabsLayerX - scWidth + 300;
							}
	
							var maintabHeader = tabsContainer.querySelector(".theplus-tabs-nav-wrapper.swiper-wrapper");
								maintabHeader.style.transform = `translate3d(-${finalVal}px, 0px, 0px)`;
						}
					}
